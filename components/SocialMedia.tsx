            ...
            <ul className='flex w-full gap-2'>
                {
                    social.map((s: SocialInt) => (
                        <motion.li initial={{ opacity: 0, y: 30 }}
                            ...
                            transition={{ delay: 0.05, type: 'spring', stiffness: 100 }} className={`h-12 w-12 flex justify-center items-center ${styles.bg_1} ${styles.shadow}`} key={s.id} ><a href={s.link} target='_blink'><img src={`/icons/${s.icon}`} className='h-6' /></a></motion.li>
                    ))
                }
            </ul>
        </>
