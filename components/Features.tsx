...
const Features = () => {
    ...
    return (
        <section className={`section-2 features container mx-auto px-4 md:px-0 ${styles.borderLine}`} >
            ...
            <div className="feature-list grid grid-cols-1 md:grid-cols-3 gap-12 w-full mt-8 " >
                {portfolio.features.map((f, i) => (
                    <motion.div initial={{ y: 150, opacity: 0 }} whileInView={{ y: 0 , opacity:  1 }} transition={{ delay: 0.2 * i }} className="feature-card w-full bg-slate-900 p-4 bg-slate-900 shadow-lg shadow-slate-900/50" key={f.id}>
