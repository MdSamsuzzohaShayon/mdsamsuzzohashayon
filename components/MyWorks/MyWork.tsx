...
const MyWork = (props: CommonPropsInt) => {
    ...
    const [works, setWorks] = useState<IProject[]>(portfolio.works);
    ...
    return (
        <section className={`section-3 my-works wrapper-con ${styles.borderLine}`} >
            ...
            <div className="work-list grid grid-cols-1 md:grid-cols-3 gap-12 w-full mt-8 " ref={workListImgs} >
                {works.map((w, i) => (
                    <motion.div initial={{ y: 150, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}
                        className="work-card w-full bg-slate-900 p-6 bg-slate-900 shadow-lg shadow-slate-900/50" key={w.id} onClick={(e) => openDialogHandler(e, w.id)}>
